...
public class Lexer {
   ...
   private Collection<Character> operators = Arrays.asList('<', '>', '=', '!');
   ...
   /**
    * Throws an exception if the current token is not the specified operator.
    * Otherwise, moves to the next token.
    * 
    * @param opr a string denoting the operator
    */
   public void eatOpr(char opr) {
      if (!matchOpr(opr))
         throw new BadSyntaxException();
      nextToken();
   }
   ...
   private void initKeywords() {
      keywords = Arrays.asList("select", "from", "where", "and", "order", "by", "asc", "desc",
            ...
            "using", "hash", "btree", "sum", "count", "avg", "min", "max", "group");
   }
}